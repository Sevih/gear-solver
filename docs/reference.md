...
| `apps/renderer/test/solver.test.ts`     | 63 tests — gem pool/score/alloc/delta (+ eligibility filter), gem override equivalence, **set-bonus hoist equivalence**, cheap ratings (+ CRC clamp, **damage-stat scaling atk/def/hp**), score normalization (+ CRC clamp), reforge sim (+ 6★ ascended budget, Talisman/EE rejection), top-K heap, STAT_TO_PRIORITY mapping, CP clamps (skills.first, ECDR) |
...
Run : `npm test --workspaces --if-present`. **Total : 108 tests** (7 parse + 63 solver + 7 gemsCapped + 8 transfer + 13 setPlans + 10 translateReco).
