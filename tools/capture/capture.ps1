...
param(
  ...
  [string]$CertDir  = $null,
  # How long to wait for /user/item after relaunching the game. A FIRST game
  # launch (patch download + login) can take ~5 min on a fresh install, so we
  # wait 6 min. Past the timeout the pipeline stays armed anyway (the capture
  # still completes when the lobby loads; the status poll picks the sentinel up).
  [int]$TimeoutSec  = 360,
  [switch]$NoRelaunch
)
